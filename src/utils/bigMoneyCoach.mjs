...
// extras   : { align, tires, asr, ro_count } from the dashboard record (optional)
export function gamePlan({ row, board, contest, entries = [], extras = {}, today = new Date() }) {
  ...
  if (goals.add_rate > 0) {
    if (row.hitRate) lines.push({ icon: '✅', tone: 'good', title: `Add rate ${pct(row.rate)} — on goal`,
      ...
      text: `From here you need the add-on on about ${pct(rateNeed.needed)} of your $50 oil changes for the remaining ${left} working day${left === 1 ? '' : 's'}. One extra customer-pay line of 0.1 hr or more is all it takes to count.` });
  }
  if (goals.hrs_ro > 0) {
  ...
  return { done, left, rateNeed, hrsNeed, leader, storeRateIf, storeHrsIf, rateTrend, hrsTrend, lines };
}
...
export function coachPrompt({ row, board, contest, entries = [], extras = {}, prizes = {}, today = new Date() }) {
  ...
  const recent = [...(entries || [])].sort((a, b) => (a.date < b.date ? 1 : -1)).slice(0, 21)
    .map(e => `  ${e.date}  addRate=${e.roh50_add_rate != null ? pct(e.roh50_add_rate) : '—'}  addlHrs/RO=${e.roh50_hrs_ro != null ? num2(e.roh50_hrs_ro) : '—'}  ROs=${e.ro_count ?? '?'}  hrs/RO=${e.hours_per_ro != null ? num2(e.hours_per_ro) : '—'}  align=${e.align != null ? pct(e.align) : '—'}  tires=${e.tires != null ? pct(e.tires) : '—'}  asr=${e.asr != null ? pct(e.asr) : '—'}`)
    .join('\n');
...
- 🎯 the single biggest lever right now, with the exact pace needed (cite the numbers)
