...
export default function TechChat({ currentUser, currentRole, hasChatAccess, refreshKey }) {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      try { msgs.forEach(m => feedMention(m, 'Tech Chat')); } catch {}
    } catch {}
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = pusher.subscribe(TECH_CHANNEL);
    ...
    return () => {
      ...
      clearInterval(poll);
      chatLive.tech = Math.max(0, chatLive.tech - 1);
    };
  }, [fetchMessages]);
  ...
  async function handleSend() {
    ...
    try {
      ...
      triggerEvent(TECH_CHANNEL, NEW_MSG_EVENT, newMsg);
    } catch (err) {
