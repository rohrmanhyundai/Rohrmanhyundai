...
// ── Advisor After Call Report ─────────────────────────────────────────────────
// Its own page, reached from the "After Call Reviews" button on the Advisor
// Calendar. It used to be bolted onto the bottom of the day form, which meant an
// advisor had to click into a calendar day to reach their surveys — surveys that
// have nothing to do with the day they happened to click.
//
// Always scoped to ONE advisor: `advisorName` is the logged-in user, or whoever
// a manager has selected on the calendar.
...
export default function AfterCallReport({ advisorName, ownAdvisor, currentRole, canEditDashboard, onBack }) {
  ...
  // No longer tied to a calendar day — the report is a live view of the surveys
  // outstanding right now, so it is stamped with today.
        ...
        {afterCallTab === 'complete' && (
          siLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, paddingTop: 16 }}>
              ...
              {pendingSurveys.map((survey, sIdx) => {
                const draft = reviewDrafts[survey.repairOrder] || {};
                ...
                return (
                  // The uploaded list can carry the same RO twice, so the index
                  // keeps the key unique — a duplicate key makes React drop or
                  // duplicate a card, i.e. a survey silently missing from review.
                  <div key={`${survey.repairOrder}-${sIdx}`} style={{
