...
export default function AdminPanel({ data, vacations, isOpen, onClose, onDataChange, currentUser }) {
  ...
  const [newVacDates, setNewVacDates] = useState('');
  const [newVacStatus, setNewVacStatus] = useState('APPROVED');
  ...
  function addVacation() {
    if (!newVacName && !newVacDates) return;
    const newVac = structuredClone(vacations);
    newVac.push({ name: newVacName.trim() || '\u2014', dates: newVacDates.trim() || '\u2014', status: newVacStatus.trim() || 'APPROVED' });
    onDataChange(data, newVac);
    setNewVacName('');
    setNewVacDates('');
    setNewVacStatus('APPROVED');
  }
  ...
  return (
    <aside className="admin open">
      ...
      <details className="edit-group">
        ...
        <div className="group-body">
          {vacations.map((v, idx) => (
            <div className="form-section" key={idx} style={{ position: 'relative', paddingBottom: 8 }}>
              <div className="vacation-edit-grid">
                ...
                <div className="field"><label>Status</label><input defaultValue={v.status || ''} onBlur={e => updateField(`vacations.${idx}.status`, e.target.value.trim() || '\u2014')} /></div>
              </div>
              <div className="actions" style={{ marginTop: 4 }}>
                <button className="secondary" onClick={() => removeVacation(idx)}>Delete Entry</button>
              </div>
            </div>
          ))}
          <div className="form-section" style={{ borderTop: '1px solid #444', paddingTop: 10, marginTop: 4 }}>
            <div className="title" style={{ marginBottom: 6 }}>Add New Vacation</div>
            <div className="vacation-edit-grid">
              <div className="field"><label>Name</label><input value={newVacName} onChange={e => setNewVacName(e.target.value)} placeholder="Employee name" /></div>
              <div className="field"><label>Dates</label><input value={newVacDates} onChange={e => setNewVacDates(e.target.value)} placeholder="e.g. Apr 14–18" /></div>
              <div className="field"><label>Status</label><input value={newVacStatus} onChange={e => setNewVacStatus(e.target.value)} placeholder="APPROVED" /></div>
            </div>
            <div className="actions" style={{ marginTop: 6 }}>
              <button onClick={addVacation} disabled={!newVacName && !newVacDates}>Add Vacation</button>
            </div>
          </div>
        </div>
      </details>
